  ...
  outputs = { self, nixpkgs, flake-utils, sops-nix, deploy-rs, disko, ... }@inputs: # Make all inputs accessible
    let
      ...
      commonNodeArgumentsFromEnv = {
        ...
        adminSshPublicKey = getEnv "ADMIN_SSH_PUBLIC_KEY" ""; # Must be provided via environment variable
    ...
    in
    {
      ...
      nixosModules = {
        ...
        hetznerK3sControlTemplate = { config, lib, pkgs, ... }: {
          imports = [
        ...
        in
        {
          default = pkgs.mkShell {
            buildInputs = with pkgs; [
              ...
              # Remove deploy-rs and sops since they're not available in pkgs
              just
              gh
              tailscale
            ];
